 ...
 */
public class SVMClassifierMatlab implements  IERPClassifier {
    private IFeatureExtraction fe;
    private static MatlabProxy proxy = null;

    public SVMClassifierMatlab() {
    ...
    public void train(List<double[][]> epochs, List<Double> targets, int numberOfiter, IFeatureExtraction fe) {
        try {
            if (proxy != null) {
                ...
                proxy.eval("model = trainsvm(out_features', out_targets')");
            }
        } catch (MatlabInvocationException e) {
            e.printStackTrace();
        }
    ...
    public double classify(double[][] epoch) {
        try {
            ...
            if (proxy != null) {
                proxy.eval("[label, score] = ClassificationSVM.predict(model, test_features');");
            }
            return ((double[])proxy.getVariable("score"))[0];
